...
# ==========================================================
# kahfimoodtzyy Full Protect + Same Admin Theme v7
#
# RULE FINAL:
# Admin utama:
# - full akses semua
...
# - boleh create server lewat panel/PTLA untuk owner user biasa/non-admin
# - TIDAK boleh create server untuk akun admin/root_admin
...
#
# Install:
#   MAIN_ADMIN_ID=1 bash kahfi-full-protect-same-theme-final-v7.sh
# atau:
#   MAIN_ADMIN_ID=1 bash kahfi-full-protect-same-theme-final-v7.sh /var/www/pterodactyl
# ==========================================================
...
print_status "Starting kahfimoodtzyy Full Protect v7..."
...
class KahfiMoodTzyPanelProtect
{
    public function handle(Request $request, Closure $next): Response
    {
         ...
         * Admin selain utama:
         * - BOLEH create server untuk owner user biasa/non-admin.
        ...
        if ($this->starts($path, 'api/application/servers')) {
            ...
            if (!$isMain) {
                if ($isCreate) {
                    ...
                    if (!$ownerId) {
                        return $this->deny($request, 'Create server wajib pilih owner user biasa.');
                    }

                    if ($this->isRootAdminUserId($ownerId)) {
                        return $this->deny($request, 'Admin selain utama tidak boleh create server untuk akun admin/root_admin.');
                    }
        ...
        /*
         * Admin selain utama boleh create server manual/PTLA,
         * tapi owner wajib user biasa/non-admin.
         */
        if ($this->isAdminServerCreate($request, $path)) {
            $ownerId = $this->requestedServerOwnerId($request);

            if (!$ownerId) {
                return $this->deny($request, 'Admin selain utama wajib pilih owner user biasa saat create server.');
            }
...
$marker = 'KAHFIMOODTZYY_PANEL_PROTECT_BOOT_V7';
...
kahfi_patch_handle_guard(
    ...
    'kahfimoodtzyy server owner non admin guard v7',
    <<<GUARD
        // kahfimoodtzyy server owner non admin guard v7
{$actor}
        // Admin selain utama boleh create server via panel/PTLA,
        // tapi owner server wajib user biasa/non-admin.
        ...
        if (\$kahfiShouldCheckServerCreate && (int) \$kahfiActorId !== \$kahfiMainAdminId) {
            ...
            if (!\$kahfiOwnerId || !is_numeric(\$kahfiOwnerId)) {
                throw new \Pterodactyl\Exceptions\DisplayException('Create server wajib pilih owner user biasa.');
            }
...
============================================================
kahfimoodtzyy FULL PROTECT v7 SELESAI
============================================================
...
- boleh create server lewat panel/PTLA untuk owner user biasa
- tidak boleh create server untuk akun admin/root_admin
...
3. Admin selain utama create server untuk user biasa via PTLA: harus bisa.
