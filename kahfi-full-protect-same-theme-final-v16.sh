...
# ==========================================================
# kahfimoodtzyy FULL PROTECT v16 CLEAN
#
# Fokus v16:
# - restore file service/provider dari backup clean dulu, supaya error parse
#   dari v13/v14/v15 lama hilang.
# - protect utama dipasang lewat middleware Laravel, bukan menanam banyak kode
#   ke service yang rawan syntax error.
#
# RULE:
# Admin utama:
# - full akses semua.
...
# - admin dashboard/client list hanya menampilkan server owner_id dia sendiri.
...
#
# Install:
# MAIN_ADMIN_ID=1 bash kahfi-full-protect-same-theme-final-v16.sh
# ==========================================================
...
info "Starting kahfimoodtzyy FULL PROTECT v16 CLEAN"
...
# Restore service/provider dari backup yang benar-benar clean untuk menghapus parse error lama.
python3 <<'PY_RESTORE_CLEAN'
...
bad_markers = [
    ...
    "KAHFI",
    "Pilih user biasa",
    "server owner admin self only guard",
    "server deletion main admin only guard",
    "user admin hard error guard",
    "block admin privilege update guard",
]
...
for target, prefixes in targets.items():
    ...
    clean = [p for p in candidates if is_clean(p)]
    ...
    if clean:
        ...
        print(f"[WARN] No clean backup found for {target}; will try to repair current file if needed.")

# Repair AppServiceProvider if no clean restore or if old bad registrations remain.
app = Path("app/Providers/AppServiceProvider.php")
if app.exists():
    s = app.read_text(errors="ignore")
    # remove old kahfi middleware lines and malformed double backslash registrations
    s = re.sub(r"\n\s*//\s*KAHFI[^\n]*\n", "\n", s)
...
class KahfiMoodTzyPanelProtect
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        // User biasa tidak boleh create PTLC, tapi file/backup/download server sendiri tetap jalan.
        ...
        // Admin selain utama boleh create PTLA/PTLC, tapi tidak boleh revoke/delete API key.
        ...
        // Admin selain utama boleh create server untuk user biasa mana pun, dan admin dia sendiri.
        ...
        // Dashboard/client list: hanya server owner_id admin itu sendiri.
        if ($this->isClientServerList($request, $path)) {
            $response = $next($request);
            return $this->filterOwnServerListResponse($request, $response);
        }

        // Direct access: hanya server owner_id admin itu sendiri.
    ...
    private function blockedAdminArea(Request $request, string $path): ?Response
    {
        ...
        if ($this->isAdminServerDelete($request, $path)) {
            return $this->deny($request, 'Delete/delsrv server hanya untuk admin utama.');
        }
...
# Patch AppServiceProvider dengan class namespace yang benar, tanpa double slash.
python3 <<'PY_PATCH_PROVIDER'
from pathlib import Path
import re
...
# Remove old Kahfi middleware lines, both valid and invalid.
s = re.sub(r"\n\s*//\s*KAHFI[^\n]*\n", "\n", s)
s = re.sub(
    r"\n\s*\$this->app\[['\"]router['\"]\]->pushMiddlewareToGroup\(['\"][^'\"]+['\"],\s*\\+Pterodactyl\\+Http\\+Middleware\\+Kahfi[^;]+::class\);\s*",
    "\n",
...
inject = "\n        // KAHFIMOODTZYY_PANEL_PROTECT_BOOT_V16\n        " + "\n        ".join(lines) + "\n"
...
file.write_text(s)
print("[OK] AppServiceProvider patched v16.")
PY_PATCH_PROVIDER
...
============================================================
kahfimoodtzyy FULL PROTECT v16 CLEAN SELESAI
============================================================
