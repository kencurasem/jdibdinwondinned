...
# ==========================================================
# kahfimoodtzyy FULL PROTECT v13
#
# RULE FINAL:
# Admin utama:
# - full akses semua.
...
# - dashboard/client API hanya boleh melihat server owner_id dia sendiri.
...
#
# Install:
# MAIN_ADMIN_ID=1 bash kahfi-full-protect-same-theme-final-v13.sh
# MAIN_ADMIN_ID=1 bash kahfi-full-protect-same-theme-final-v13.sh /var/www/pterodactyl
# ==========================================================
...
print_status "Starting kahfimoodtzyy FULL PROTECT v13"
...
# Bersihkan middleware lama dari eksperimen sebelumnya.
...
class KahfiMoodTzyPanelProtect
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        /*
         * Application API / PTLA:
         * - Create server boleh untuk admin selain utama, tapi:
         *   owner admin/root_admin hanya boleh diri sendiri.
         * - List/detail/update/delete server hanya admin utama.
         */
        ...
        /*
         * Application API / PTLA user:
         * Admin selain utama boleh create user biasa, tapi tidak boleh admin/root_admin.
         */
        ...
        /*
         * User biasa:
         * Tidak boleh create PTLC, tapi file/download/backup server sendiri tetap jalan.
         */
        ...
        /*
         * Admin selain utama:
         * Boleh create PTLA/PTLC, tidak boleh revoke/delete API key.
         */
        ...
        /*
         * Admin selain utama boleh create server:
         * - user biasa mana pun: boleh.
         * - akun admin/root_admin dia sendiri: boleh.
         * - admin/root_admin lain termasuk admin utama: ditolak.
         */
        ...
        /*
         * Dashboard/client server list untuk admin selain utama:
         * hanya tampilkan server owner_id dia sendiri.
         * Server milik user biasa yang dia buat tidak akan tampil.
         */
         ...
         * Admin selain utama hanya boleh ke server yang owner_id miliknya sendiri.
         */
    ...
    private function blockedAdminArea(Request $request, string $path): ?Response
    {
        ...
        /*
         * Nodes dan Nests diprotect.
         * AJAX/JSON GET tetap boleh untuk kebutuhan form create server kalau panel membutuhkannya.
         */
        ...
        /*
         * Jangan tampilkan list semua server di admin panel.
         * Kalau buka /admin/servers, arahkan ke create server.
         */
...
print_success "Middleware v13 dibuat."

# Patch AppServiceProvider: hapus registrasi middleware lama lalu inject middleware v13.
php <<'PHP_PATCH'
<?php

$file = 'app/Providers/AppServiceProvider.php';
$class = '\\Pterodactyl\\Http\\Middleware\\KahfiMoodTzyPanelProtect::class';
$marker = 'KAHFIMOODTZYY_PANEL_PROTECT_BOOT_V13';
...
/*
 * Remove old Kahfi middleware registrations to avoid duplicate/bentrok.
 */
$s = preg_replace('/\n\s*\/\/\s*KAHFI[^\n]*\n/s', "\n", $s);
...
echo "AppServiceProvider patched v13.\n";
PHP_PATCH

# Bersihkan legacy blocker dari versi lama yang menolak semua root_admin owner.
php <<'PHP_CLEAN_LEGACY'
<?php

$file = 'app/Services/Servers/ServerCreationService.php';
if (!is_file($file)) {
    echo "ServerCreationService not found, skip legacy clean.\n";
    exit(0);
}
...
/*
 * Replace old throw message inside an existing:
 * if (!empty($kahfiOwner->root_admin)) { throw ...Pilih user biasa... }
 * with self-admin exception.
 */
$oldThrows = [
    ...
    'throw new \\Pterodactyl\\Exceptions\\DisplayException("Admin selain utama tidak boleh create server untuk akun admin/root_admin. Pilih user biasa.");',
    "throw new \\Pterodactyl\\Exceptions\\DisplayException('Admin selain utama tidak boleh create server untuk akun admin/root_admin.');",
    'throw new \\Pterodactyl\\Exceptions\\DisplayException("Admin selain utama tidak boleh create server untuk akun admin/root_admin.");',
];

$newThrow = "if ((int) (\\$kahfiOwner->id ?? 0) !== (int) (\\$kahfiActorId ?? (\\$kahfiAuthUser->id ?? 0))) { throw new \\Pterodactyl\\Exceptions\\DisplayException('Admin selain utama tidak boleh create server untuk admin/root_admin lain termasuk admin utama.'); }";

foreach ($oldThrows as $old) {
    $s = str_replace($old, $newThrow, $s);
}

/*
 * Disable very old guard condition that still blocks all root_admin owners.
 * It is safe because v13 adds the correct guard below.
 */
$s = preg_replace(
    '/if\s*\(\s*\$kahfiShouldCheckServerCreate\s*&&\s*\(int\)\s*\$kahfiActorId\s*!==\s*\$kahfiMainAdminId\s*\)\s*\{/',
    'if (false && $kahfiShouldCheckServerCreate && (int) $kahfiActorId !== $kahfiMainAdminId) {',
    $s
);

if ($s !== $before) {
    file_put_contents($file, $s);
    echo "Cleaned legacy ServerCreationService blockers.\n";
} else {
    echo "No legacy ServerCreationService blocker found.\n";
}
PHP_CLEAN_LEGACY

# Patch service-level guards.
php <<'PHP_PATCH_SERVICES'
<?php
...
kahfi_patch_handle_guard(
    ...
    'kahfimoodtzyy server owner admin self only guard v13',
    <<<GUARD
        // kahfimoodtzyy server owner admin self only guard v13
{$actor}
...
kahfi_patch_handle_guard(
    ...
    'kahfimoodtzyy server deletion main admin only guard v13',
    <<<GUARD
        // kahfimoodtzyy server deletion main admin only guard v13
{$actor}
...
kahfi_patch_handle_guard(
    ...
    'kahfimoodtzyy user admin hard error guard v13',
    <<<GUARD
        // kahfimoodtzyy user admin hard error guard v13
{$actor}
...
kahfi_patch_handle_guard(
    ...
    'kahfimoodtzyy block admin privilege update guard v13',
    <<<GUARD
        // kahfimoodtzyy block admin privilege update guard v13
{$actor}
...
# Patch Application API server controllers supaya list/detail/update/delete tidak lolos jika middleware group tidak kena.
php <<'PHP_PATCH_APP_API'
<?php
...
function kahfi_patch_named_methods(string $file, array $methods, string $marker, string $guard): void
{
    ...
    $s = file_get_contents($file);
    if (strpos($s, $marker) !== false) {
        echo "Already app-api patched: {$file}\n";
        return;
    }
    ...
    if ($count > 0 && $patched !== null) {
        ...
        echo "Patched app-api server guard: {$file} ({$count})\n";
    }
}

$guard = <<'GUARD'
        // kahfimoodtzyy application server no-list-no-delete guard v13
        $kahfiMainAdminId = (int) env('KAHFI_MAIN_ADMIN_ID', 1);
        $kahfiAuthUser = \Illuminate\Support\Facades\Auth::user();
        if (!$kahfiAuthUser) {
            try { $kahfiAuthUser = request()->user(); } catch (\Throwable $e) { $kahfiAuthUser = null; }
        ...
        if (!$kahfiActorId) {
            try {
                foreach (request()->attributes->all() as $kahfiAttr) {
                    if (is_object($kahfiAttr) && isset($kahfiAttr->user_id)) { $kahfiActorId = (int) $kahfiAttr->user_id; break; }
                    if (is_array($kahfiAttr) && isset($kahfiAttr['user_id'])) { $kahfiActorId = (int) $kahfiAttr['user_id']; break; }
                }
            } catch (\Throwable $e) {}
        }
        ...
        if (!$kahfiActorId) {
            try {
                ...
                $kahfiCandidates = [];
                foreach ([16, 20, 24, 32, 8] as $kahfiLen) {
                    if (strlen($kahfiRaw) >= $kahfiLen) $kahfiCandidates[] = substr($kahfiRaw, 0, $kahfiLen);
                }
                foreach (array_values(array_unique(array_filter($kahfiCandidates))) as $kahfiIdentifier) {
                    $kahfiRow = \Illuminate\Support\Facades\DB::table('api_keys')->where('identifier', $kahfiIdentifier)->first();
                    if ($kahfiRow && isset($kahfiRow->user_id)) { $kahfiActorId = (int) $kahfiRow->user_id; break; }
                }
            } catch (\Throwable $e) {}
        }

        if ((int) $kahfiActorId !== $kahfiMainAdminId) {
            throw new \Pterodactyl\Exceptions\DisplayException('List/detail/update/delete/delsrv server lewat PTLA/PTLC hanya untuk admin utama.');
        }
GUARD;
...
foreach (['app/Http/Controllers/Api/Application/Servers', 'app/Http/Controllers/Api/Application'] as $dir) {
    ...
    foreach ($it as $fileInfo) {
        ...
        if (stripos($file, 'Server') !== false || stripos($contents, 'Pterodactyl\\Models\\Server') !== false || stripos($contents, 'ServerTransformer') !== false) {
            kahfi_patch_named_methods($file, $methods, 'kahfimoodtzyy application server no-list-no-delete guard v13', $guard);
        }
    }
}
PHP_PATCH_APP_API

# Theme badge tetap ada nama kahfimoodtzyy.
...
if [ "$BAD" = "1" ]; then
    print_error "Ada syntax error. Backup ada di $BACKUP_DIR"
    exit 1
fi
...
============================================================
kahfimoodtzyy FULL PROTECT v13 SELESAI
============================================================
