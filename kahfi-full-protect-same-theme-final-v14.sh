...
# ==========================================================
# kahfimoodtzyy FULL PROTECT v14
#
# RULE FINAL:
# Admin utama:
# - full akses semua.
...
#
# Install:
# MAIN_ADMIN_ID=1 bash kahfi-full-protect-same-theme-final-v14.sh
# MAIN_ADMIN_ID=1 bash kahfi-full-protect-same-theme-final-v14.sh /var/www/pterodactyl
# ==========================================================
...
info "Starting kahfimoodtzyy FULL PROTECT v14"
...
# Hapus middleware lama yang sering bentrok.
...
class KahfiMoodTzyPanelProtect
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        // User biasa tidak boleh create PTLC, tapi akses file/backup/download server sendiri tetap bawaan panel.
...
ok "Middleware v14 dibuat."

php <<'PHP_PATCH_APP_PROVIDER'
<?php

$file = 'app/Providers/AppServiceProvider.php';
$class = '\\Pterodactyl\\Http\\Middleware\\KahfiMoodTzyPanelProtect::class';
$marker = 'KAHFIMOODTZYY_PANEL_PROTECT_BOOT_V14';
...
/*
 * Remove old Kahfi registrations to avoid bentrok/double middleware.
 */
$s = preg_replace('/\n\s*\/\/\s*KAHFI[^\n]*\n/s', "\n", $s);
...
echo "AppServiceProvider patched v14.\n";
PHP_PATCH_APP_PROVIDER

# Bersihkan blocker lama "Pilih user biasa" dari ServerCreationService.
php <<'PHP_CLEAN_OLD'
<?php

$file = 'app/Services/Servers/ServerCreationService.php';
if (!is_file($file)) {
    echo "ServerCreationService tidak ditemukan, skip clean old blocker.\n";
    exit(0);
}
...
$newThrow = <<<'CODE'
if ((int) ($kahfiOwner->id ?? 0) !== (int) ($kahfiActorId ?? ($kahfiAuthUser->id ?? 0))) {
...
if ($s !== $before) {
    file_put_contents($file, $s);
    echo "Cleaned old self-admin blocker.\n";
} else {
    echo "No old self-admin blocker found.\n";
}
PHP_CLEAN_OLD
...
kahfi_patch_handle_guard(
    ...
    'kahfimoodtzyy server owner admin self only guard v14',
    <<<GUARD
        // kahfimoodtzyy server owner admin self only guard v14
{$actor}
...
kahfi_patch_handle_guard(
    ...
    'kahfimoodtzyy server deletion main admin only guard v14',
    <<<GUARD
        // kahfimoodtzyy server deletion main admin only guard v14
{$actor}
...
kahfi_patch_handle_guard(
    ...
    'kahfimoodtzyy user admin hard error guard v14',
    <<<GUARD
        // kahfimoodtzyy user admin hard error guard v14
{$actor}
...
kahfi_patch_handle_guard(
    ...
    'kahfimoodtzyy block admin privilege update guard v14',
    <<<GUARD
        // kahfimoodtzyy block admin privilege update guard v14
{$actor}
...
============================================================
kahfimoodtzyy FULL PROTECT v14 SELESAI
============================================================
